...
| 3 | La base no se puede volver a montar desde el repositorio | ✅ resuelto — `schema.sql` completo, probado en Postgres |
...
| 11 | 8 funciones SQL con `search_path` mutable; ejecutables con la llave anónima | ✅ en `schema.sql` — falta correrlo en producción |
...
| 15 | `.env.example` citado pero inexistente; `.gitignore` no cubre `.env` | ✅ resuelto |
...
| 17 | Lector de iPhone desde CDN | 🟡 mitigado — se precarga al abrir la app; falta traerlo al repositorio |

## Encontrados y resueltos durante la consolidación

| # | Problema | Estado |
|---|---|---|
| 18 | `barcodes.json` descartaba los 34 códigos que comparten dos variantes: escanear un bebedero WT214 o una puerta 16GC10 daba "no encontrado" | ✅ resuelto |
| 19 | Al escanear se veía el número de parte, no el producto (el pedido original) | ✅ resuelto — nombre al escanear, en el aviso y en la búsqueda |
| 20 | Un Excel de códigos cargado desde la app solo llegaba a ese aparato | ✅ resuelto — se guarda en el mapa, solo lo distinto |
| 21 | El escáner decía "cámara denegada" para cualquier falla | ✅ resuelto |
| 22 | `yard_acquire_lock` devolvía "sí" y "no" a la vez (faltaba `return`) | ✅ en `schema.sql` — falta correrlo en producción |
| 23 | `00000000000 → MF`: un escaneo de ceros resolvía a un producto | ✅ resuelto |

## Sobre la rama `claude/yard-map-barcode-conversion-b7e4k3`

**Consolidada el 23 de septiembre.** La rama se fundió con `main` quedándose con
su árbol exacto (7 funciones, su tema, sus 63 commits), y encima se aplicó solo
lo que `main` no tenía: los puntos 18 a 23 de arriba y el 3, 11 y 15. No se
trajo el sistema visual "Galvanized" de la rama vieja: `main` ya tiene
`tarter-brand-theme`, y dos sistemas de estilos encimados es justamente el
problema del #17 de la revisión anterior.
